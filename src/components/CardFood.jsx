import React from 'react';
import HoverBg from "../assets/hover-bg.jpg";


const CardFood = ({
  src,
  title,
  itemCount,
  color,
  className,
  type = 'category',
  name,
  subName,
}) => {
  if (type === 'trending') {
    return (
      <div className="relative group">
        <div className={`flex flex-col gap-2.5 w-full md:w-[300px] md:h-[300px] ${color} shadow-[0px_11px_39px_0px_#00000012] rounded-[17px] px-6 py-7.5`}>
          <img src={src} className='w-[120px]' />
          <h1 className="text-[26px] font-medium">
            {name}
          </h1>
          <p className="text-primary text-lg font-bold">
            {subName}
          </p>

          <div className="flex items-center gap-0.5">
            {[...Array(4)].map((_, index) => (
              <svg key={`filled-${index}`} width="12" height="11" viewBox="0 0 12 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M11.3124 4.08243C11.2303 3.82962 10.9849 3.65586 10.6393 3.60566L7.89801 3.20728L6.67209 0.723295C6.51748 0.410063 6.2764 0.230408 6.01062 0.230408C5.74484 0.230408 5.50376 0.410063 5.34918 0.723295L4.1233 3.20726L1.38201 3.60564C1.03632 3.65584 0.790968 3.82962 0.708824 4.08238C0.6267 4.33517 0.723066 4.61996 0.973164 4.86377L2.95677 6.79725L2.48849 9.5275C2.42692 9.88663 2.53149 10.1014 2.63008 10.2183C2.7454 10.3551 2.91362 10.4304 3.10377 10.4304C3.24753 10.4304 3.40064 10.3883 3.55874 10.3051L6.01062 9.01604L8.4625 10.3051C8.62065 10.3882 8.77373 10.4304 8.91751 10.4304H8.91755C9.10772 10.4304 9.27595 10.3551 9.39126 10.2183C9.48984 10.1014 9.59443 9.88661 9.53283 9.52746L9.0645 6.79727L11.0481 4.86381C11.2982 4.62 11.3946 4.33519 11.3124 4.08243Z"
                  fill="#FF8412" />
              </svg>
            ))}
            <svg width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g clipPath="url(#clip0_307_5436)">
                <path d="M10.9711 4.08243C10.889 3.82962 10.6436 3.65586 10.2979 3.60566L7.5567 3.20728L6.33078 0.723295C6.17617 0.410063 5.93509 0.230408 5.66931 0.230408C5.40354 0.230408 5.16245 0.410063 5.00787 0.723295L3.78199 3.20726L1.0407 3.60564C0.69501 3.65584 0.449659 3.82962 0.367515 4.08238C0.285391 4.33517 0.381758 4.61996 0.631855 4.86377L2.61546 6.79725L2.14718 9.5275C2.08561 9.88663 2.19018 10.1014 2.28878 10.2183C2.40409 10.3551 2.57231 10.4304 2.76247 10.4304C2.90622 10.4304 3.05933 10.3883 3.21743 10.3051L5.66931 9.01604L8.12119 10.3051C8.27934 10.3882 8.43242 10.4304 8.5762 10.4304H8.57624C8.76641 10.4304 8.93464 10.3551 9.04995 10.2183C9.14853 10.1014 9.25312 9.88661 9.19153 9.52746L8.72319 6.79727L10.7068 4.86381C10.9569 4.62 11.0533 4.33519 10.9711 4.08243Z" fill="#C9C9C9" />
              </g>
              <defs>
                <clipPath id="clip0_307_5436">
                  <rect width="10.6609" height="10.6609" fill="white" transform="translate(0.338867)" />
                </clipPath>
              </defs>
            </svg>
          </div>
        </div>
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <img
            src={HoverBg}
            alt="hover background"
            className="w-full md:w-[300px] h-[300px] object-cover rounded-[17px] mix-blend-multiply opacity-10"
          />
        </div>
      </div>
    );
  }

  return (
    <div
      className={`
            relative
            flex items-center justify-center 
            ${color} rounded-lg 
            w-[230px] 
            h-[150px]
            group hover:h-[170px]
            transition-all duration-300 ease-in-out
          `}
    >
      <div className={`
            absolute inset-0
            flex items-center justify-center
            ${color} rounded-lg
          `}>
        <div className="flex flex-col items-center justify-center gap-3 bg-wh">
          <div className="w-12 h-12 flex items-center justify-center">
            <img
              src={src}
              alt={title}
              className={`w-full h-full object-contain ${className}`}
            />
          </div>
          <div>
            <h3 className="font-medium text-dark">{title}</h3>
            <p className="text-sm text-dark font-normal">{itemCount} Items</p>
          </div>
        </div>
      </div>
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <img
          src={HoverBg}
          alt="hover background"
          className="w-[300px] h-[170px] object-cover rounded-[17px] mix-blend-multiply opacity-10"
        />
      </div>
    </div>
  );
};

export default CardFood;